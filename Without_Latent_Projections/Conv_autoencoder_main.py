    ...
    for i in range(Files_to_execute_for.shape[0]):
        ...
        for j in range(5):
            ...
            if my_config.classif_mode==0:
                ...
                mydatas.write_means()
                #res=mydatas.Calculate_WGSS(mydatas.data,my_config.classif_mode)
                #mydatas.write_means(0,res,' WGSS_estimation.tsv')
                aligned=mydatas.Encoder_layer.predict(mydatas.data[:,1:])
                aligned=mydatas.Decoder_layer.predict(aligned)
                class_labels=mydatas.data[:,0].reshape(mydatas.data.shape[0],1)
                aligned=npy.concatenate((class_labels,aligned),axis=1)
